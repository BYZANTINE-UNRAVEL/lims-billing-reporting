...
@Component({
  ...
  template: `
    <section class="commission-workspace">
      <div class="toolbar">
        <div class="date-field"><span>From</span><input type="date" [(ngModel)]="filters.from" (change)="load()"></div>
        <div class="arrow">→</div>
        <div class="date-field"><span>To</span><input type="date" [(ngModel)]="filters.to" (change)="load()"></div>
        ...
        <button class="btn secondary" (click)="currentMonth()">Current month</button>
...
export class CommissionsPageComponent implements OnInit, OnDestroy {
  ...
  iso(d:Date){const y=d.getFullYear(),m=String(d.getMonth()+1).padStart(2,'0'),day=String(d.getDate()).padStart(2,'0');return `${y}-${m}-${day}`;}
  currentMonth(reload=true){const n=new Date();this.filters.from=this.iso(new Date(n.getFullYear(),n.getMonth(),1));this.filters.to=this.iso(n);if(reload)this.load();}
